...
**Memory system** (`memory/`): Separated into archives (append-only JSONL, never compressed) and active context (markdown, regenerated by daily synthesis workflow):
- `memory/learnings.jsonl` — archive of self-reflections (append-only, structured JSONL with day/ts/source/title/context/takeaway)
- `memory/social_learnings.jsonl` — archive of insights about humans from discussions (append-only, JSONL with day/ts/source/who/insight)
- `memory/active_learnings.md` — active context loaded into prompts (regenerated by `.github/workflows/synthesize.yml`)
- `memory/active_social_learnings.md` — active social context loaded into prompts (regenerated by synthesis workflow)
