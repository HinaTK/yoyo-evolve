...
- 证据进度：forward logs=10，matured days=1，forward samples=0。

## 重点标的表
- `688981.SH`：状态=avoid，主题=china-a-semiconductors，置信度=0.29，风险上限=avoid。
  - 理由：Draft avoid generated from deterministic ranking and edge gate fields for 688981.SH.
  - 风险：symbol_risk_veto；cn_limit_up_chase_block；nontechnical_score_below_action_min
  - 非技术面：nontechnical_evidence status=available, total_score=0.534, event_risk=policy, flags=['nontechnical_score_below_action_min', 'event_risk_policy']
...
- `512480.SH`：状态=avoid，主题=china-a-semiconductors，置信度=0.28，风险上限=avoid。
  ...
  - 非技术面：nontechnical_evidence status=available, total_score=0.629, event_risk=policy, flags=['event_risk_policy']
  - 失效条件：Invalidate if price loses MA20 support, volume confirmation fades, or the edge/cost gate is no longer met.
...
- `588000.SH`：等确认；状态=watch_only；score=85.95；非正式行动候选。
  ...
  - 失效条件：Invalidate if price loses MA20 support, volume confirmation fades, or the edge/cost gate is no longer met.
- `9618.HK`：等确认；状态=ranking_only；score=77.32；非正式行动候选。
  - why：接近候选，但仍有关键确认项未满足。
  - 主要障碍：非技术面证据过期、非技术分不足、政策风险。
  - 升级条件：阻断项清零、正式门槛通过、事件风险回落且正式资料接入后，可升级为「可考虑研究」。
  - 失效条件：若观察分数跌破门槛、状态转弱或新增硬阻断，移出重点跟踪。
- `0941.HK`：等确认；状态=ranking_only；score=75.43；非正式行动候选。
  - why：接近候选，但仍有关键确认项未满足。
  - 主要障碍：非技术面证据过期、非技术分不足。
  - 升级条件：阻断项清零、正式门槛通过、事件风险回落且正式资料接入后，可升级为「可考虑研究」。
  - 失效条件：若观察分数跌破门槛、状态转弱或新增硬阻断，移出重点跟踪。
- `159915.SZ`：等确认；状态=ranking_only；score=71.03；非正式行动候选。
  - why：接近候选，但仍有关键确认项未满足。
  - 主要障碍：量能不足、市场位置偏高。
  - 升级条件：阻断项清零、正式门槛通过、事件风险回落且正式资料接入后，可升级为「可考虑研究」。
  - 失效条件：若观察分数跌破门槛、状态转弱或新增硬阻断，移出重点跟踪。
- `600900.SH`：等确认；状态=ranking_only；score=62.18；非正式行动候选；正式资料未接入，不清除正式行动门槛。
  - why：正式资料未接入或存在资料阻断，不能视为正式行动候选。
  - 主要障碍：正式资料未接入、正式证据缺失、市场位置偏高、成本/边际不足。
  - 升级条件：阻断项清零、正式门槛通过、事件风险回落且正式资料接入后，可升级为「可考虑研究」。
  - 失效条件：若观察分数跌破门槛、状态转弱或新增硬阻断，移出重点跟踪。
- `515790.SH`：等确认；状态=ranking_only；score=60.36；非正式行动候选。
  - why：接近候选，但仍有关键确认项未满足。
  - 主要障碍：政策风险、市场位置偏高、成本/边际不足。
  - 升级条件：阻断项清零、正式门槛通过、事件风险回落且正式资料接入后，可升级为「可考虑研究」。
  - 失效条件：若观察分数跌破门槛、状态转弱或新增硬阻断，移出重点跟踪。
- `510500.SH`：继续观察；状态=ranking_only；score=69.34；非正式行动候选。
  - why：有跟踪价值，但当前强度、证据或关卡状态不足。
  - 主要障碍：量能不足、市场位置偏高。
  - 升级条件：分数进入观察线、状态改善且主要障碍减少后，可升级为「等确认」。
  - 失效条件：若出现硬阻断、状态失败或分数持续偏低，则降为「暂不碰」。
- `0857.HK`：继续观察；状态=ranking_only；score=67.57；非正式行动候选。
  - why：有跟踪价值，但当前强度、证据或关卡状态不足。
...
## Gate 拒绝与观察重点
- `688981.SH` score=93.79 action=False watch=True gates=['cn_limit_up_chase_block', 'nontechnical_score_below_action_min', 'event_risk_policy']
- `512480.SH` score=87.81 action=False watch=False gates=['event_risk_policy', 'market_range_pos_60_above_action_limit', 'same_theme_best_peer_evidence_missing_or_failed']
- `588000.SH` score=85.95 action=False watch=True gates=['event_risk_policy', 'market_range_pos_60_above_action_limit']
...
## 非技术面证据与归因
- 非技术面证据覆盖：正式证据=39 / 标的数=46；正式资料未接入=7；缺失=0；阻断项=29；严重项=0。
