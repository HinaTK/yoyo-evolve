# Investment Strategy Backtest

Generated: `2026-05-06T11:01:35Z`
Strategy: `l5_mvp_conservative_v1`
Samples: `66`
Production samples: `66`
Promotable samples: `66`
Sample quality: `sufficient`
Strict samples: `66`
Relaxed samples: `0`
Qualified samples: `66`
Diagnostic layer samples: `270`
Diagnostic-only samples: `270`
Diagnostic samples: `270`
Average net return: `-0.028`%
Win rate: `0.424`
Average benchmark alpha: `0.792`%
Average max adverse return: `-0.305`%
Max adverse-ish return: `-6.557`%
Adverse breach rate: `0.0`

## Weights
- `trend_weight`: 0.45
- `momentum_weight`: 0.3
- `range_weight`: 0.15
- `risk_penalty_weight`: 1.15

## Recent Records
- `2026-03-10` rank 1 `1299.HK`: net=-4.845%, alpha=-2.738%, adverse=-4.495%, qualified=True, diagnostic=False
- `2026-03-11` rank 1 `0883.HK`: net=4.568%, alpha=4.841%, adverse=3.706%, qualified=True, diagnostic=False
- `2026-03-12` rank 1 `0883.HK`: net=0.818%, alpha=0.861%, adverse=1.168%, qualified=True, diagnostic=False
- `2026-03-13` rank 1 `0883.HK`: net=-1.425%, alpha=-2.63%, adverse=-1.075%, qualified=True, diagnostic=False
- `2026-03-16` rank 1 `0883.HK`: net=0.126%, alpha=1.854%, adverse=-3.872%, qualified=True, diagnostic=False
- `2026-03-19` rank 1 `0883.HK`: net=2.76%, alpha=7.225%, adverse=2.705%, qualified=True, diagnostic=False
- `2026-03-20` rank 1 `0883.HK`: net=0.045%, alpha=3.76%, adverse=0.395%, qualified=True, diagnostic=False
- `2026-03-23` rank 1 `0883.HK`: net=-4.547%, alpha=-5.978%, adverse=-6.557%, qualified=True, diagnostic=False
- `2026-03-24` rank 1 `2331.HK`: net=-1.805%, alpha=-0.824%, adverse=-2.455%, qualified=True, diagnostic=False
- `2026-03-25` rank 1 `9618.HK`: net=0.203%, alpha=3.125%, adverse=0.553%, qualified=True, diagnostic=False
- `2026-04-01` rank 1 `0006.HK`: net=2.075%, alpha=-0.159%, adverse=1.374%, qualified=True, diagnostic=False
- `2026-04-08` rank 1 `9618.HK`: net=0.007%, alpha=1.273%, adverse=-0.447%, qualified=True, diagnostic=False
- `2026-04-09` rank 1 `2331.HK`: net=-2.292%, alpha=-1.711%, adverse=-1.942%, qualified=True, diagnostic=False
- `2026-04-14` rank 1 `9618.HK`: net=5.889%, alpha=5.323%, adverse=4.593%, qualified=True, diagnostic=False
- `2026-04-15` rank 1 `9618.HK`: net=1.058%, alpha=-0.192%, adverse=1.408%, qualified=True, diagnostic=False
- `2026-04-16` rank 1 `9618.HK`: net=-1.481%, alpha=-0.981%, adverse=-1.131%, qualified=True, diagnostic=False
- `2026-04-21` rank 1 `0941.HK`: net=-0.111%, alpha=2.107%, adverse=-0.358%, qualified=True, diagnostic=False
- `2026-04-23` rank 1 `0857.HK`: net=2.893%, alpha=3.091%, adverse=2.072%, qualified=True, diagnostic=False
- `2026-04-24` rank 1 `0857.HK`: net=0.797%, alpha=1.224%, adverse=1.147%, qualified=True, diagnostic=False
- `2026-04-27` rank 1 `0941.HK`: net=-1.416%, alpha=-0.38%, adverse=-1.066%, qualified=True, diagnostic=False
